...
import (
	...
	pkg "github.com/obiwan007/usersrv/pkg"
)
...
func main() {
	...
	p := &pkg.CommandParams{tlsForGrpc, balancer, certFile, keyFile, port, zipkin, "orderservice"}

	opts, collector := pkg.PrepareOptsTracing(p)
	defer collector.Close()

	conn, _, err := pkg.CreateClient(p, *storeAddr)
	defer conn.Close()
